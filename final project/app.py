...
loans = pd.read_csv(r'./hr_clean.csv')
...
def database():
    data= pd.read_csv("C:/Users/William/Downloads/Purwadhika Data Science/Final Project Purwadhika/Final Project WillyWonka/HR Analytics Job Change of Data Scientists/final project/hr_clean.csv").head(1000)
    data.index.name = None
    titles = " "
   
        # data.to_html()
    return render_template('database.html', tables = [data.to_html(classes = 'data', header = 'true')], titles = titles)
...
if __name__ == '__main__':
    ...
    filename = r'./dtc_final.sav'
    
    model = pickle.load(open(filename, 'rb'))
    app.run(debug=True, port=8000) # 8000